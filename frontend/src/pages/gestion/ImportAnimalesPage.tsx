...
export default function ImportAnimalesPage() {
  ...
  const handleDownloadTemplate = async () => {
    const blob = await importsApi.downloadAnimalTemplate();
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "plantilla_animales.xlsx";
    a.click();
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setValidating(true);
    try {
      const result = await importsApi.validateAnimales(file);
      setPreview(result);
      setPhase("preview");
    } catch (err: any) {
  ...
  const handleConfirm = async () => {
    ...
    setConfirming(true);
    try {
      const res = await importsApi.confirmAnimales(preview.temp_key);
      setJobId(res.job_id);
      setPhase("processing");
    } catch (err: any) {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Importar Animales</h1>
      ...
      {phase === "upload" && (
        <div className="card space-y-4">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-700 space-y-1">
            <p className="font-medium text-blue-800">Columnas del Excel:</p>
        ...
        <div className="space-y-4">
          <div className="card">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="text-center bg-gray-50 rounded-lg p-3">
                ...
                <div className="text-xs text-gray-500">Total animales</div>
              </div>
              ...
              <div className="border border-red-200 bg-red-50 rounded-lg p-3 mb-4">
                ...
                <ul className="space-y-1 max-h-40 overflow-y-auto">
                  {preview.errores.map((e) => (
                    <li key={e.fila} className="text-xs text-red-700">
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              Importar {preview.filas_ok} animales
            </button>
          </div>
